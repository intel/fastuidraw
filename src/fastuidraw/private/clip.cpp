...
namespace
{
  ...
  bool
  compute_clip_dist(const fastuidraw::vec3 &clip_eq,
                    const fastuidraw::vec3 &pt)
  {
    return fastuidraw::dot(pt, clip_eq);
  }

  inline
  fastuidraw::vec3
  compute_intersection(const fastuidraw::vec3 &p0, float d0,
                       const fastuidraw::vec3 &p1, float d1)
  {
    float t;
  ...
  template<typename T>
  bool
  clip_against_planeT(const fastuidraw::vec3 &clip_eq,
                      fastuidraw::c_array<const T> pts,
  ...
  template<typename T>
  bool
  clip_against_planesT(fastuidraw::c_array<const fastuidraw::vec3> clip_eq,
                       fastuidraw::c_array<const T> in_pts,
                       unsigned int *out_idx,
                       fastuidraw::vecN<std::vector<T>, 2> &scratch_space)
  {
