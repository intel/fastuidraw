...
draw_scene(bool with_clipping)
{
  ...
  m_painter->fill_rect(PainterData(&brush), vec2(0.0f, 0.0f), vec2(wh.x(), wh.y()));
  ...
  m_painter->fill_rect(PainterData(&brush), vec2(wh) * 0.0f, vec2(wh) * 0.5f);
  ...
  m_painter->fill_rect(PainterData(&brush), vec2(wh) * r * 0.25f, vec2(wh));
}
