...
/*!
 * \brief
 * A ArcStrokedPath represents the data needed to draw a path stroked.
 * In contrast to StrokedPath, an ArcStrokedPath will use arc-edges
 * to approximate the edges of a Path, resulting in far few vertices.
 * It contains -all- the data needed to stroke a path regardless of
 * stroking style. In particular, for a given TessellatedPath,
 * one only needs to construct a ArcStrokedPath <i>once</i> regardless
...
class ArcStrokedPath:
    public reference_counted<ArcStrokedPath>::non_concurrent
{
public:
  /*!
   * \brief
   * Opaque object to hold work room needed for functions
   * of ArcStrokedPath that require scratch space.
   */
  ...
  public:
    ...
    /*!
     * The list of chunks to take from ArcStrokedPath::edges()
   ...
   * edges or all edges of the non-closing edges of a ArcStrokedPath.
   */
  enum chunk_selection
    {
      /*!
       * Select the chunk that holds all the edges of
       * ONLY the non-closing edges of the ArcStrokedPath
       */
      all_non_closing = StrokedCapsJoins::all_non_closing,

      /*!
       * Select the chunk that holds all the edges of
       * ONLY the closing edges of the ArcStrokedPath
       */
      all_closing = StrokedCapsJoins::all_closing,
    };

  /*!
   * Ctor. Construct a ArcStrokedPath from the data
   * of a TessellatedPath.
   * \param P source ArcTessellatedPath
   */
  explicit
  ArcStrokedPath(const ArcTessellatedPath &P);
  ...
  /*!
   * Returns the StrokedCapsJoins of the ArcStrokedPath that
   * provides the attribute data for the stroking of the
   * joins and caps.
   */
