 ...
 */
    ...
    namespace code
    {
       ...
       * glyph is sheared a great deal. The _adaptive variants compute the
       * shearing and if the glyph is not sheared by a great deal will compute
       * with the non-hq variant and if it is sheared greatly will compute
       * with the hq variant. The variants with additional out arguments also
       * output in those additional out arguments various data values that
       * are computed in doing the computation that a debug version of a
       * shader can visualize the structure of the glyph.
