...
class painter_cells:public sdl_cairo_demo
{
public:
  painter_cells(void);
...
private:
  ...
  static
  void
  generate_random_colors(int count, std::vector<color_t> &out_values, bool force_opaque);
  ...
  command_line_argument_value<double> m_table_width, m_table_height;
  ...
  command_line_argument_value<double> m_pixel_size;
  command_line_argument_value<double> m_fps_pixel_size;
  ...
  command_line_argument_value<double> m_min_x_velocity, m_max_x_velocity;
  command_line_argument_value<double> m_min_y_velocity, m_max_y_velocity;
  ...
  command_line_argument_value<double> m_change_stroke_width_rate;
...
derived_init(int w, int h)
{
  ...
  m_table_params.m_line_color = color_t(1.0, 1.0, 1.0, 1.0);
  ...
  m_cell_shared_state.m_stroke_width = double(m_init_stroke_width.m_value);
  ...
  if(m_init_show_all_table.m_value)
    {
      ScaleTranslate sc;
      sc.scale(1.0 / std::max(twh.x(), twh.y()));
  ...
  m_text_brush = color_t(1.0, 0.0, 1.0, 1.0);
  m_text_size = m_fps_pixel_size.m_value;
...
update_cts_params(void)
{
  ...
  double speed = double(m_draw_timer.restart()) * double(0.001);
